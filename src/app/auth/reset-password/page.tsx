...
const ERROR_MESSAGES: Record<string, string> = {
  INVALID_TOKEN: '유효하지 않은 링크입니다. 비밀번호 찾기를 다시 시도해주세요.',
  EXPIRED:       '링크가 만료되었습니다(1시간). 비밀번호 찾기를 다시 시도해주세요.',
  ALREADY_USED:  '이미 사용된 링크입니다. 비밀번호 찾기를 다시 시도해주세요.',
...
function ResetPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(ERROR_MESSAGES[code] ?? '재설정 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <PageSection id="reset-password" />
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4">
        <div className="w-full max-w-md">
          <Link to="/auth/login" className="mb-6 inline-flex items-center text-sm text-gray-600 hover:text-gray-900">
              ...
              <>
                ...
                {!token ? (
                  <div className="rounded-md bg-red-50 px-4 py-3 text-sm text-red-600">
                    유효하지 않은 링크입니다.{' '}
                    <Link to="/auth/forgot-password" className="font-medium underline">비밀번호 찾기를 다시 시도해주세요.</Link>
                  </div>
                ) : (
